...
def checkout_git_repo_and_run_build_hook(git_repository, git_commitish, passthrough_arguments):
    git_checkout_dir = irods_python_ci_utilities.git_clone(git_repository, git_commitish)
    return irods_python_ci_utilities.subprocess_get_output(['python', 'irods_consortium_continuous_integration_build_hook.py'] + passthrough_arguments, cwd=git_checkout_dir, check_rc=True)
...
def main():
    ...
    rc, stdout, stderr = checkout_git_repo_and_run_build_hook(module.params['git_repository'], module.params['git_commitish'], module.params['passthrough_arguments'])
